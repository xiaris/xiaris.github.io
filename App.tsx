...
import { Mail, Github, GraduationCap, Sparkles } from 'lucide-react';
import PROFILE_IMAGE from './image.jpg';

const getCurrentView = () => {
  return window.location.hash.startsWith('#/showcase') ? 'showcase' : 'home';
};

const getShowcaseSectionId = () => {
  ...
  return (
    <div className="bg-void text-slate-200 min-h-screen selection:bg-cosmic-blue/30 selection:text-white font-sans">
        ...
        <section id="research" className="py-24 px-6 md:px-16 lg:px-24 max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-16 gap-6">
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:gap-6">
              ...
              <a
                href="#/showcase"
                className="group inline-flex items-center gap-2 rounded-full border border-cosmic-blue/30 bg-cosmic-blue/10 px-4 py-2 text-sm font-medium text-cosmic-blue transition-all duration-300 hover:border-cosmic-blue/50 hover:bg-cosmic-blue/20 hover:text-white"
              >
      ...
      <footer className="py-8 text-center text-slate-600 text-sm border-t border-slate-900">
        <p>&copy; {new Date().getFullYear()} Ruisong Xia. Built with AI STUDIO.</p>
      </footer>
    </div>
  );
};
...
const App: React.FC = () => {
  const [view, setView] = useState<'home' | 'showcase'>(() => getCurrentView());

  useEffect(() => {
    const handleHashChange = () => {
      setView(getCurrentView());
      const sectionId = getShowcaseSectionId();
  ...
  return view === 'showcase' ? <ShowcasePage /> : <HomePage />;
};
